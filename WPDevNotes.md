...
Next, **we need to define a root element by creating a variable** (we'll call it `rootEl`, but we can choose any name) ***(the main container where our React app will be rendered)***, **to specify where we want to render our element**.
